...
void SceneBase::RenderText(Mesh* mesh, std::string text, Color color, float space)
{
	...
	for(unsigned i = 0; i < text.length(); ++i)
	{
		...
		characterSpacing.SetToTranslation(i * space, 0, 0); //1.0f is the spacing of each character, you may change this value
...
void SceneBase::RenderMeshIn2D(Mesh *mesh, bool enableLight, float size, Vector3 pos)
{
	Mtx44 ortho;
	ortho.SetToOrtho(-80, 80, -60, 60, -20, 20);
	projectionStack.PushMatrix();
	projectionStack.LoadMatrix(ortho);
	viewStack.PushMatrix();
	viewStack.LoadIdentity();
