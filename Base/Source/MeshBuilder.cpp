...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color, int sides, float length)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	Vertex v;

	float degreePerSlice = 360.f / sides;
	//float radianPerSlice = Math::DegreeToRadian(360.f) / numSlice;
	for (unsigned slice = 0; slice < sides + 1; ++slice)
	{
		float theta = slice * degreePerSlice;
		v.color = color;
		v.normal.Set(0, 1, 0);
		v.pos.Set(length * cos(Math::DegreeToRadian(theta)), 0, length * sin(Math::DegreeToRadian(theta)));
		vertex_buffer_data.push_back(v);

	}
	for (unsigned slice = 0; slice < sides; ++slice)
	{
